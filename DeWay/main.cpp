...
float camX= -1435.0f,camY=54,camZ=120.0f;
float x= -1435.0f,y=54,z=125.0f;
float red= 1.0f, blue=1.0f, green=1.0f;
float xAngle, yAngle = 0.0f;
float comp_jf_x=0,comp_jf_y=0,comp_jf_z=0,comp_xAngle=0;
int move=0,comp_plane_view=0;
float car_view=0,car_x=0,car_z=0,car_xAngle=0,car_gear=0;
float dom_x=0,dom_z=0,dom_x2=0,dom_z2=0;
float dd=0.0,bb=0.0;
float metro_train_distance_1=0.0 ,metro_train_speed1=0.0,normal_y=10;
float plane_y_movement = 0, speed_plane_y_movement=0,go_forward=0,plane_stat=0;
int plane_turning_xAngle=0;
...
int main(int argc,char **argv)
{
    ...
    glutCreateWindow("Mars Colony (Copyright. 2017)");
    
    glutDisplayFunc(renderScene);
    glutSpecialFunc(keyboard);
    glutIdleFunc(renderScene);
    glutReshapeFunc(reshape);
    glClearColor(0.933, 0.910, 0.667,1);
...
void text() //Draw Instructions onto screen
{
    char controls1[512];
    char controls2[512];
    char controls3[512];
    ...
    sprintf(controls1,"W-A-S-D = Move/Turn           "); //Movement
    sprintf(controls2, "SPACE = Fly Up/Take off            "); //Up
    sprintf(controls3, "~ = Fly Down/Land             "); //Down
    sprintf(controls4, "P = Plane                    "); //Plane
    sprintf(controls5, "M/N = Ride Train             "); //Train
    sprintf(controls6, "C = Car                      "); //Car
    sprintf(controls7, "ESC = Exit all Vehicles      "); //Exit
    sprintf(controls8, "2-6 = Plane Tricks (GO-TO PARADE GROUNDS)  "); //Tricks
    sprintf(controls9, "1 = Stop All Plane Tricks    "); //Stop Tricks
    sprintf(controls10, "0 = Fly in Back of Plane    "); //Fly in other seat
    
    
    //Draw Characters:
    
    
    int len, lon, lin, lo, l, w, o, p, j, k;

    
    len = strlen(controls1);
    lon = strlen(controls2);
    lin = strlen(controls3);
    lo = strlen(controls4);
    l = strlen(controls5);
    w = strlen(controls6);
    o = strlen(controls7);
    ...
    for ( int i = 0; i < len; ++i ) //Draws every character
    {
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls1[i]);
        
    }
    
    glRasterPos2i(10, 530);

    for ( int i = 0; i < lon; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls2[i]);
        
    }
    
    glRasterPos2i(10, 510);

    for ( int i = 0; i < lin; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls3[i]);
        
    }
    
    glRasterPos2i(10, 490);

    for ( int i = 0; i < lo; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls4[i]);
        
    }
    
    glRasterPos2i(10, 470);

    for ( int i = 0; i < l; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls5[i]);
        
    }
    
    glRasterPos2i(10, 450);

    for ( int i = 0; i < w; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls6[i]);
        
    }
    
    glRasterPos2i(10, 430);

    for ( int i = 0; i < o; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls7[i]);
        
    }
    
    glRasterPos2i(10, 410);

    for ( int i = 0; i < p; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls8[i]);
        
    }
    
    glRasterPos2i(10, 390);
    
    for ( int i = 0; i < j; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls9[i]);
        
    }
    
    glRasterPos2i(10, 370);
    
    for ( int i = 0; i < k; ++i ) //Draws every character
    {
        
        
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, controls10[i]);
...
void compound() //Containment hangars for Planes
{
...
void watch_tower() //Small rectangular watch tower
{
...
void watch_tower2() //Large Cone watch tower
{
    glColor3f(0.5,0.5,0.5);
    glRotatef(90,1,0,0);
    glTranslatef(-14,5,- 60);
    glutSolidCone(15,100,30,60);
    glColor3f(1.0,1.0,1.0);
    glTranslatef(0,0,-1);
    glutSolidCone(22.5,30,30,60);
    glColor3f(0.1,0.1,0.1);
    glutWireCone(22.5001,30,30,60);

    
}

void runway() //Main Runway
{
...
void runway_strips() //Runway Middle Lines
{
...
void runway_long_strips() //Runway Side lines
{
    glColor3f(0.9,0.9,0.9);
    //broken strips
    glBegin(GL_TRIANGLES);
...
void fighter_plane() //Fighter Plane
{
    glBegin(GL_TRIANGLES);
    //nose 1
    glColor3f(0.5,0.5,0.5);
...
void parade_ground() //Parade Grounds decor
{
    glTranslatef(0,1.3,0);
...
void main_building()
{
    ...
    glVertex3f(0,-19.9, 400);
    glVertex3f(-200,-19.9,400);
    ...
    //main building extension beside metro_train tracks:

    
    //Wall close to tracks
    glColor3f(0.88,0.88,0.88);
    glVertex3f(820,-21.9,400);
    glVertex3f(-200,-21.9, 400);
    glVertex3f(-200, 40,400);
    
    glVertex3f(-200, 40,400);
    glVertex3f(820,-21.9,400);
    glVertex3f(820, 40,400);
    
    glVertex3f(-1300,-21.9,400);
    glVertex3f(-200,-21.9, 400);
    glVertex3f(-200, 40,400);
    
    glVertex3f(-200, 40,400);
    glVertex3f(-1300,-21.9,400);
    glVertex3f(-1300, 40,400);
    

    //Wall away from tracks
    glVertex3f(820,-21.9,350);
    glVertex3f(-200,-21.9, 350);
    glVertex3f(-200, 40,350);
    
    glVertex3f(-200, 40,350);
    glVertex3f(820,-21.9,350);
    glVertex3f(820, 40,350);
    
    glVertex3f(-1300,-21.9,350);
    glVertex3f(-200,-21.9, 350);
    glVertex3f(-200, 40,350);
    
    glVertex3f(-200, 40,350);
    glVertex3f(-1300,-21.9,350);
    glVertex3f(-1300, 40,350);
    
    
    //Wall endings and Roof
    ...
    glColor3f(0.92,0.92,0.92);
    glVertex3f(-100,0,220);
    glVertex3f(-100,0, 270);
    glVertex3f(-150,0,270);
    
    glVertex3f(-150,0,270);
    glVertex3f(-100,0,220);
    glVertex3f(-150,0,220);
    //military wall 1
...
void renderScene()
{
    ...
    if(comp_plane_view==7)
    {
        ...
        if(plane_stat==2)
        {
            if(plane_turning_xAngle>=0 )
            {
                plane_turning_xAngle-=4;
            }
        ...
        if(go_forward>=0.5 && y>=3)
        {
            ...
            y+=plane_y_movement;camY+=plane_y_movement;
            
        }
        float fraction = go_forward;
        x+=camX*fraction;z+=camZ*fraction;
        glLoadIdentity();
        
        glTranslatef(0,-1.9,-40);
        glScaled(1.8,1.3,1.2);
        glRotatef(180,0,1,0);
        glRotatef(plane_turning_xAngle,0,0,1);
        glPushMatrix();
        fighter_plane();
        glPopMatrix();
    ...
    glRotatef(-90,0,1,0);
    metro_train_distance_1+=0.81;
    if(metro_train_distance_1>=0 && metro_train_distance_1 <=240)
    {
        metro_train_speed1+=0;
    }
    else if(metro_train_distance_1>=750 && metro_train_distance_1 <=990)
    {
        metro_train_speed1+=0;
    }
    else if(metro_train_distance_1>=1650 && metro_train_distance_1 <=1890)
    {
        metro_train_speed1+=0;
    }
    else if(metro_train_distance_1>=1900)
    {
        metro_train_speed1=0;
        metro_train_distance_1=0;
    }
    else
    {
        metro_train_speed1+=0.81;
    }
    glTranslatef(0,0,metro_train_speed1);
    train();
    //bus 2
...
void keyboard1(unsigned char key1,int xx1,int yy1)
{
    ...
    if(key1=='a')
    {
        ...
        if(!inAir){
            ...
            if(plane_turning_xAngle % 360 == 0){
                
                plane_turning_xAngle = 0;
            }
